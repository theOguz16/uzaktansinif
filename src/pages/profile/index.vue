  ...
  methods: {
      ...
      if (this.tyt.trim() !== "") {
        try {
          ...
          await axiosInstance.post(
            `https://api.fizikodev.com/users/${this.user._id}/tyt-net`,
            {
    ...
    async aytListEkle() {
      if (this.ayt.trim() !== "") {
        try {
          // Backend'e TYT netini eklemek için POST isteği gönder
          await axiosInstance.post(
            `https://api.fizikodev.com/users/${this.user._id}/ayt-net`,
            {
    ...
    async getOdevler() {
      try {
        ...
        const { data } = await axiosInstance.get(
          `https://api.fizikodev.com/profile/odevler`
        );
    ...
    async getProfileData() {
      try {
        ...
        //soruları çekmece
        const { data } = await axiosInstance.get(
          `https://api.fizikodev.com/profile/sorular`
        );
    ...
    async soruSil(itemToDelete) {
      try {
        const response = await axiosInstance.delete(
          `https://api.fizikodev.com/profile/sorular/${itemToDelete._id}`
        );
    ...
    async removeTodo(itemToDelete) {
      try {
        const response = await axiosInstance.delete(
          `https://api.fizikodev.com/profile/odevler/${itemToDelete._id}`
        );
